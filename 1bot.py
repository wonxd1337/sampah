...
def get_running_packages():
    # Ambil list semua package roblox yang terinstal
    ...
    for pkg in installed_packages:
        ...
        # Pastikan output PID tidak kosong
        if pid_out:
            # Ambil PID pertama jika ada banyak PID yang dikembalikan
            first_pid = pid_out.split()[0]
            
            # Langkah 2: Gunakan PID untuk membaca file cgroup
            cgroup_out = run_root(f"cat /proc/{first_pid}/cgroup")
            
            # Langkah 3: Cek apakah ada output cpuset:/foreground
            if "cpuset:/foreground" in cgroup_out:
                running.append(pkg)
        ...
        try:
            ...
            if isinstance(commands, dict) and commands:
                for pkg, cmd_info in commands.items():
                    ...
                    # Tambahan LOG supaya terlihat di Termux bot sedang bekerja mengeksekusi ini
                    if cmd in ["START", "STOP", "RERUN"]:
                        print(f"[{time.strftime('%H:%M:%S')}] Executing {cmd} command for {pkg} ...")
                    ...
                    if cmd == "START":
                        ...
                        force_stop(pkg)
                        time.sleep(2.0) # Jeda sedikit ditingkatkan menjadi 2 detik agar Force Stop sempurna
                        start_game(pkg, mode, target)
                        ack_execution(pkg)
                    elif cmd == "IDLE":
                        ack_execution(pkg)
